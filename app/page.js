...
export default function Dashboard(){
  ...
  // Fetch pending invites and shared projects for the current user
  useEffect(() => {
    ...
    fetch('/api/my-invites', {
      ...
      .then(r => r.json())
      .then(data => {
        if (data.pending) setPendingInvites(data.pending);
        if (data.accepted) setSharedProjects(data.accepted);
      })
      .catch(() => {});
  }, [user]);
  ...
  async function acceptInvite(inv) {
    try {
      const res = await fetch('/api/invite-accept', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: inv.token, userId: user?.id }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      setPendingInvites(prev => prev.filter(p => p.id !== inv.id));
      window.location.href = '/player?project=' + inv.project_id;
    } catch (err) {
  ...
  async function declineInvite(inv) {
    try {
      const res = await fetch('/api/invite-delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ collaboratorId: inv.id }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      setPendingInvites(prev => prev.filter(p => p.id !== inv.id));
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {pendingInvites.length > 0 && (<div style={{marginBottom:'32px'}}>
          <h2 style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:'20px',fontWeight:400,marginBottom:'16px',color:'var(--text)'}}>Pending Invites</h2>
          {pendingInvites.map(inv => (
            <div key={inv.id} style={{display:'flex',alignItems:'center',gap:'16px',padding:'16px',background:'var(--surf)',border:'1px solid var(--border)',borderRadius:'12px',marginBottom:'8px'}}>
              {inv.project?.image_url && <img src={inv.project.image_url} style={{width:'48px',height:'48px',borderRadius:'8px',objectFit:'cover'}} alt="" />}
              <div style={{flex:1}}>
                <div style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:'16px',color:'var(--text)'}}>{inv.project?.title || 'Untitled'}</div>
                {inv.project?.artist && <div style={{fontSize:'12px',color:'var(--t2)',marginTop:'2px'}}>{inv.project.artist}</div>}
              </div>
              <button onClick={()=>acceptInvite(inv)} style={{padding:'10px 20px',background:'var(--amber)',border:'none',borderRadius:'8px',color:'#000',fontFamily:"'DM Mono',monospace",fontSize:'12px',fontWeight:600,cursor:'pointer'}}>Accept</button>
              <button onClick={()=>declineInvite(inv)} style={{padding:'10px 20px',background:'transparent',border:'1px solid var(--border)',borderRadius:'8px',color:'var(--t2)',fontFamily:"'DM Mono',monospace",fontSize:'12px',cursor:'pointer'}}>Decline</button>
            </div>
          ))}
        </div>)}
        ...
        <div className="grid">
          ...
          :projects.filter(p=>{if(!searchTerm.trim())return true;const q=searchTerm.trim().toLowerCase();return(p.title||'').toLowerCase().includes(q)||(p.artist||'').toLowerCase().includes(q);}).map((p,idx)=>(<div key={p.id} style={{position:'relative'}}><ProjectCard project={p} idx={idx} onDelete={deleteProject} onSave={handleSave} unreadCount={noteCounts[p.id]||0} role="creator"/>{noteCounts[p.id]>0&&<span style={{position:'absolute',top:8,right:8,background:'#e8a020',color:'#000',borderRadius:'50%',minWidth:22,height:22,display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,zIndex:10,pointerEvents:'none'}}>{noteCounts[p.id]}</span>}<div style={{position:'absolute',top:306,left:10,display:'flex',alignItems:'center',gap:4,background:'rgba(0,0,0,0.6)',padding:'4px 10px 4px 7px',borderRadius:10,zIndex:2,pointerEvents:'none'}}><svg viewBox="0 0 16 16" style={{width:12,height:12,fill:p.user_id===user?.id?'#e8a020':'#5dcaa5',flexShrink:0}}>{p.user_id===user?.id?<path d="M8 1l2 4 4.5.7-3.3 3.1.8 4.5L8 11.3 3.9 13.3l.8-4.5L1.5 5.7 6 5z"/>:<><circle cx="6" cy="5" r="3"/><circle cx="11" cy="5" r="3"/><path d="M0 14c0-3 2.7-5 6-5 .7 0 1.4.1 2 .3C6.8 10.2 6 11.5 6 13v1H0zm8 0c0-2.2 1.3-4 3-4s3 1.8 3 4H8z"/></>}</svg><span style={{fontSize:11,color:p.user_id===user?.id?'#e8a020':'#5dcaa5',fontFamily:'monospace',letterSpacing:'0.4px'}}>{p.user_id===user?.id?'creator':'collaborator'}</span></div></div>))}
        </div>
      </div>
