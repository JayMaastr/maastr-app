    ...
    <>
      ...
      <div className="app">
        ...
        <div className="grid">
          ...
          :projects.filter(p=>{if(!searchTerm.trim())return true;const q=searchTerm.trim().toLowerCase();return(p.title||'').toLowerCase().includes(q)||(p.artist||'').toLowerCase().includes(q);}).map((p,idx)=>(<div key={p.id} style={{position:'relative'}}><ProjectCard project={p} idx={idx} onDelete={deleteProject} onSave={handleSave} unreadCount={noteCounts[p.id]||0} role="creator"/>{noteCounts[p.id]>0&&<span style={{position:'absolute',top:8,right:8,background:'#e8a020',color:'#000',borderRadius:'50%',minWidth:22,height:22,display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,zIndex:10,pointerEvents:'none'}}>{noteCounts[p.id]}</span>}<div style={{position:'absolute',bottom:6,left:12,display:'flex',alignItems:'center',gap:4,pointerEvents:'none'}}><svg viewBox="0 0 16 16" style={{width:12,height:12,fill:'#e8a020',flexShrink:0}}><path d="M8 1l2 4 4.5.7-3.3 3.1.8 4.5L8 11.3 3.9 13.3l.8-4.5L1.5 5.7 6 5z"/></svg><span style={{fontSize:10,color:'#e8a020',fontFamily:'monospace',letterSpacing:'0.3px'}}>creator</span></div></div>))}
        </div>
      </div>
