...
@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
    ...
    <div className="ps-controls-bar">
      {project?.image_url&&<div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}><img className="ps-art" src={project.image_url} alt=""/><div style={{display:"flex",flexDirection:"column",gap:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600,color:"var(--t1)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontFamily:"var(--fh)"}}>{project.title}</div>{project.artist&&<div style={{fontSize:11,color:"var(--t2)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontFamily:"var(--fm)"}}>{project.artist}</div>}</div></div>}
