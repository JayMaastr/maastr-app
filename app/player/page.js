...
/* Brighter color swatches Ã¢ÂÂ warmth (col) x loudness (row opacity) */
const TONE_BG=[
...
function Waveform({peaks, progress, notes, duration, onSeek}) {
  ...
  // Main draw effect Ã¢ÂÂ runs on mount and when peaks change
  useEffect(() => {
    ...
    function draw() {
      ...
      ctx.fillText('Processing audioÃ¢ÂÂ¦ wonÃ¢ÂÂt be long', W / 2, H / 2);
      ctx.beginPath();
    ...
    // ResizeObserver Ã¢ÂÂ just let the draw loop handle resize naturally
...
/* ToneGrid Ã¢ÂÂ bright color swatches, no text in cells, X through used (color kept) */
function ToneGrid({value,usedTones=[],onChange,onSetAll,showSetAll}){
  ...
  return(<div className="tgm-wrap">
    <div className="tgm-axes"><span>Ã¢ÂÂ Warmer</span><span style={{margin:'0 auto',color:'var(--amber)',fontWeight:500,fontSize:10}}>TONE GRID</span><span>Brighter Ã¢ÂÂ</span></div>
    ...
    {usedTones.length>0&&<div style={{fontSize:10,color:'var(--t3)',marginTop:6}}>Ã¢ÂÂ Already mastered Ã¢ÂÂ crossed cells unavailable</div>}
    ...
    <div className={'td-modal'+(open?' td-open':'')}>
      <div className="td-header">
        ...
        <div className="td-title-block">
          ...
          {displayRev&&<div className="td-rev-row"><span className="td-rev-badge">{displayRev.label||'v1'}</span>{displayRev.tone_label&&<span className="td-tone-badge">{displayRev.tone_label}</span>}{revisions.length>1&&<button className="td-rev-switch" onClick={()=>setRevSwitcherOpen(v=>!v)}>{revisions.length} versions Ã¢ÂÂ¾</button>}</div>}
        </div>
      </div>
      ...
      {/* Master selector Ã¢ÂÂ shows preset pills for active revision */}
        ...
        <div className="td-master-pills">
          {[
            ...
            >
              ...
              {isPending&&<span className="td-master-pill-status">Ã¢ÂÂ³</span>}
              {isReady&&!isActive&&<span className="td-master-pill-status">Ã¢ÂÂ</span>}
              {isActive&&<span className="td-master-pill-status">Ã¢ÂÂ¶</span>}
            </button>);
          })}
        </div>
        {activeMaster&&<button className="td-master-clear" onClick={()=>onMasterSelect(null)}>
          Ã¢ÂÂ Back to original
        </button>}
      </div>}
...
const readyMaster=activeRevision?.masters?.find(m=>m.status==='ready'&&m.hls_url); useEffect(()=>{
    ...
    el.pause();
    if(hlsUrl){
      // HLS path Ã¢ÂÂ instant start, FLAC quality
      ...
      // WAV fallback Ã¢ÂÂ direct stream
      el.src=wavUrl; el.load();
    }
    return ()=>{
      if(window.__hlsInst){window.__hlsInst.destroy();window.__hlsInst=null;}
  ...
  // HLS polling Ã¢ÂÂ watches for hls_url to appear after encoding completes
  useEffect(()=>{
...
useEffect(()=>{setActiveSource('mix');},[activeTrackId]);
  ...
  return(<>
    ...
    /* TONE GRID Ã¢ÂÂ color swatches, X on used */
...
@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
    <div className="topbar"><div style={{display:'flex',alignItems:'center',gap:6,minWidth:0}}><a href="/" className="logo">maastr<em>.</em></a><span style={{color:'var(--border2)',fontSize:14,flexShrink:0}}>/</span><span className="breadcrumb">{project?.title||'Ã¢ÂÂ¦'}</span></div><div style={{display:'flex',alignItems:'center',gap:8}}><a href="/" className="back">Ã¢ÂÂ Dashboard</a>{user&&<NotificationCenter user={user}/>}<div style={{position:'relative'}}>
    ...
    <div className="ps-controls-bar">
      {readyMaster&&(<div className="ps-source-toggle"><button className={`ps-src-btn${activeSource==='mix'?' ps-src-active':''}`} onClick={()=>setActiveSource('mix')}>MIX</button><button className={`ps-src-btn${activeSource==='master'?' ps-src-active':''}`} onClick={()=>setActiveSource('master')}>MASTER</button></div>)}{!readyMaster&&activeRevision?.masters?.length>0&&(<span className="ps-mastering-lbl">Masteringâ¦</span>)}<div className="ps-transport">
        ...
        <div style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',background:'var(--surf)',border:'1px solid var(--border2)',borderRadius:14,padding:28,width:360,maxWidth:'90vw',zIndex:201}}>
          ...
          {reportSent
            ? <div style={{textAlign:'center',padding:'20px 0'}}>
                ...
                <div style={{fontSize:13,color:'var(--amber)'}}>Thanks Ã¢ÂÂ we got it.</div>
