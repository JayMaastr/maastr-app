    ...
    <div className={'td-modal'+(open?' td-open':'')}>
      ...
      <div className="td-comments-scroll">
        {notes.length>0?notes.map(n=>(<div key={n.id} data-comment-id={n.id} className="td-comment-card" onClick={()=>{setActiveNoteId(activeNoteId===n.id?null:n.id);n.timestamp_sec!=null&&onSeekToTime(n.timestamp_sec)}} style={activeNoteId===n.id?{background:'#1a1a1a',borderRadius:'8px',margin:'0 -4px',padding:'10px 8px'}:{}}>
          ...
          {activeNoteId===n.id&&(<div style={{display:'flex',gap:'12px',marginTop:'6px'}}><span style={{fontSize:'10px',color:'#fbbf24',cursor:'pointer'}}>↩ Reply</span>{n.author_name===(user?.email?.split('@')[0]||'You')?(<><span style={{fontSize:'10px',color:'#888',cursor:'pointer'}}>✎ Edit</span><span style={{fontSize:'10px',color:'#f87171',cursor:'pointer'}}>✕ Delete</span></>):(!n.resolved&&<span style={{fontSize:'10px',color:'#4ade80',cursor:'pointer'}}>✓ Mark complete</span>)}{n.resolved&&<span style={{fontSize:'9px',background:'rgba(74,222,128,0.12)',color:'#4ade80',padding:'1px 6px',borderRadius:'3px'}}>✓ Addressed</span>}</div>)}
