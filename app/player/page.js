...
@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
    <div className="topbar"><div style={{display:'flex',alignItems:'center',gap:6,minWidth:0}}><Link href="/" className="logo">maastr<em>.</em></Link><span style={{color:'var(--border2)',fontSize:14,flexShrink:0}}>/</span><span className="breadcrumb">{project?.title||''}</span></div><div style={{display:'flex',alignItems:'center',gap:8}}><Link href="/" className="back"> Dashboard</Link>{user&&<NotificationCenter user={user}/>}<div style={{position:'relative'}}>
      ...
      {activeTrack?(<><div className="ps-track-info">{project?.artist&&<span style={{fontSize:11,color:'var(--t3)',whiteSpace:'nowrap',flexShrink:0}}>{project.artist}</span>}{project?.artist&&<span style={{fontSize:11,color:'var(--t3)',margin:'0 5px',flexShrink:0}}>·</span>}{project?.title&&<span style={{fontSize:11,color:'var(--t2)',whiteSpace:'nowrap',flexShrink:0}}>{project.title}</span>}{project?.title&&<span style={{fontSize:11,color:'var(--t3)',margin:'0 5px',flexShrink:0}}>·</span>}<span className="ps-track-name">{activeTrack.title}</span>{<div style={{display:'flex',alignItems:'center',gap:'2px',background:'var(--surf2)',borderRadius:'20px',padding:'3px',marginLeft:'auto',flexShrink:0}}><button style={{fontSize:'10px',fontWeight:700,letterSpacing:'.06em',padding:'4px 12px',borderRadius:'17px',border:'none',transition:'all .2s',background:activeSource==='mix'?'var(--amber)':'transparent',color:activeSource==='mix'?'#000':'rgba(255,255,255,.4)',cursor:'pointer'}} onClick={()=>setActiveSource('mix')}>MIX</button><button disabled={!readyMaster} style={{fontSize:'10px',fontWeight:700,letterSpacing:'.06em',padding:'4px 12px',borderRadius:'17px',border:'none',transition:'all .2s',background:activeSource==='master'&&readyMaster?'var(--amber)':'transparent',color:readyMaster?(activeSource==='master'?'#000':'rgba(255,255,255,.4)'):'rgba(255,255,255,.18)',cursor:readyMaster?'pointer':'default'}} onClick={()=>{if(readyMaster)setActiveSource('master');}}>MASTER</button></div>}</div><div className="ps-waveform"><Waveform peaks={activeSource==='master'&&readyMaster?.peaks?readyMaster.peaks:activeTrack.peaks} progress={progress} notes={notes} duration={duration} onSeek={handleSeek}/><div className="ps-time-row"><span>{fmt(currentTime)}</span><span>{fmt(duration)}</span></div></div></>):(<div className="ps-no-track-top">Tap a track to start listening</div>)}
    </div>
    <div className="page">
