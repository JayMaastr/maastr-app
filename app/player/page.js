...
function Waveform({ peaks, progress, notes, duration, onSeek }) {
  ...
  useEffect(() => {
    ...
    const BAR=2, GAP=1, STEP=BAR+GAP, numBars=Math.floor(W/STEP), cy=H/2;
    ...
    if (notes&&notes.length&&duration>0) {
      notes.forEach(n=>{
        ...
        nctx.save(); nctx.strokeStyle='rgba(255,255,255,0.3)'; nctx.lineWidth=1; nctx.setLineDash([2,3]);
        nctx.beginPath(); nctx.moveTo(x,4); nctx.lineTo(x,H-4); nctx.stroke(); nctx.restore();
    ...
    function draw() {
      ...
      if (Math.abs(playX-lastPlayX)>=0.5) {
        ...
        if (prog>0.001) {
          ...
          ctx.save();
          ctx.shadowColor='rgba(232,160,32,0.9)'; ctx.shadowBlur=10;
          ctx.fillStyle='#ffffff'; ctx.fillRect(px-1,0,2,H);
          ctx.fillStyle='#ffcc44';
          ctx.beginPath(); ctx.arc(px,3,5,0,Math.PI*2); ctx.fill();
          ctx.restore();
        }
      }
    ...
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .proj-artist{font-size:12px;color:var(--t2);margin-bottom:20px;letter-spacing:.04em;}
        ...
        .tab-btn:hover,.tab-btn:active{color:var(--text);border-color:var(--t2);}
        ...
        .note-bar{background:var(--surf);border:1px solid var(--border2);border-radius:12px;padding:16px;margin-bottom:24px;}
        ...
        textarea{width:100%;background:var(--bg);border:1.5px solid var(--border2);border-radius:8px;padding:10px 12px;color:var(--text);font-family:var(--fm);font-size:12px;resize:none;outline:none;min-height:64px;transition:border-color .15s;-webkit-appearance:none;} textarea:focus{border-color:var(--amber);}
