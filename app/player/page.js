...
/* Brighter color swatches ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ warmth (col) x loudness (row opacity) */
const TONE_BG=[
...
function Waveform({peaks, progress, notes, duration, onSeek}) {
  ...
  // Main draw effect ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ runs on mount and when peaks change
  useEffect(() => {
    ...
    function draw() {
      ...
      ctx.fillText('Processing audioÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦ wonÃÂÃÂ¢ÃÂÃÂÃÂÃÂt be long', W / 2, H / 2);
      ctx.beginPath();
    ...
    // ResizeObserver ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ just let the draw loop handle resize naturally
...
/* ToneGrid ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ bright color swatches, no text in cells, X through used (color kept) */
function ToneGrid({value,usedTones=[],onChange,onSetAll,showSetAll}){
  ...
  return(<div className="tgm-wrap">
    <div className="tgm-axes"><span>ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Warmer</span><span style={{margin:'0 auto',color:'var(--amber)',fontWeight:500,fontSize:10}}>TONE GRID</span><span>Brighter ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ</span></div>
    ...
    {usedTones.length>0&&<div style={{fontSize:10,color:'var(--t3)',marginTop:6}}>ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Already mastered ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ crossed cells unavailable</div>}
...
function TrackDetail({open,track,activeRevision,notes,currentTime,duration,progress,isPlaying,onTogglePlay,onSkip,onPrevTrack,onNextTrack,canPrev,canNext,onSeek,onClose,onPost,onSeekToTime,onRevisionSelect,activeMaster,onMasterSelect}){
  ...
  return(<>
    ...
    <div className={'td-modal'+(open?' td-open':'')}>
      ...
      {/* Master selector ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ shows preset pills for active revision */}
        ...
        <div className="td-master-pills">
          {[
            ...
            >
              ...
              {isPending&&<span className="td-master-pill-status">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ³</span>}
              {isReady&&!isActive&&<span className="td-master-pill-status">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ</span>}
              {isActive&&<span className="td-master-pill-status">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¶</span>}
            </button>);
          })}
        </div>
        {activeMaster&&<button className="td-master-clear" onClick={()=>onMasterSelect(null)}>
          ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Back to original
        </button>}
      </div>}
...
const readyMaster=activeRevision?.masters?.find(m=>m.status==='ready'&&m.hls_url); useEffect(()=>{
    ...
    el.pause();
    if(hlsUrl){
      // HLS path ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ instant start, FLAC quality
      ...
      // WAV fallback ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ direct stream
      el.src=wavUrl; el.load();
    }
    return ()=>{
      if(window.__hlsInst){window.__hlsInst.destroy();window.__hlsInst=null;}
  ...
  // HLS polling ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ watches for hls_url to appear after encoding completes
  useEffect(()=>{
...
useEffect(()=>{setActiveSource('mix');},[activeTrackId]);
  ...
  return(<>
    ...
    /* TONE GRID ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ color swatches, X on used */
...
@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
    <div className="topbar"><div style={{display:'flex',alignItems:'center',gap:6,minWidth:0}}><a href="/" className="logo">maastr<em>.</em></a><span style={{color:'var(--border2)',fontSize:14,flexShrink:0}}>/</span><span className="breadcrumb">{project?.title||'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¦'}</span></div><div style={{display:'flex',alignItems:'center',gap:8}}><a href="/" className="back">ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Dashboard</a>{user&&<NotificationCenter user={user}/>}<div style={{position:'relative'}}>
    ...
    <div className="ps-controls-bar">
      {readyMaster&&(<div style={{display:'flex',gap:'2px',background:'var(--surf2)',borderRadius:'20px',padding:'3px',marginRight:'8px'}}><button style={{...{fontSize:'10px',fontWeight:700,letterSpacing:'.06em',padding:'4px 12px',borderRadius:'17px',border:'none',cursor:'pointer',transition:'all .2s'},background:activeSource==='mix'?'var(--amber)':'transparent',color:activeSource==='mix'?'#000':'rgba(255,255,255,.4)'}} onClick={()=>setActiveSource('mix')}>MIX</button><button style={{...{fontSize:'10px',fontWeight:700,letterSpacing:'.06em',padding:'4px 12px',borderRadius:'17px',border:'none',cursor:'pointer',transition:'all .2s'},background:activeSource==='master'?'var(--amber)':'transparent',color:activeSource==='master'?'#000':'rgba(255,255,255,.4)'}} onClick={()=>setActiveSource('master')}>MASTER</button></div>)}{!readyMaster&&activeRevision?.masters?.length>0&&(<span style={{fontSize:'10px',color:'rgba(255,255,255,.4)',letterSpacing:'.06em',marginRight:'8px'}}>Masteringâ¦</span>)}<div className="ps-transport">
        ...
        <div style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',background:'var(--surf)',border:'1px solid var(--border2)',borderRadius:14,padding:28,width:360,maxWidth:'90vw',zIndex:201}}>
          ...
          {reportSent
            ? <div style={{textAlign:'center',padding:'20px 0'}}>
                ...
                <div style={{fontSize:13,color:'var(--amber)'}}>Thanks ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ we got it.</div>
