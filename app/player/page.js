...
useEffect(()=>{setActiveSource('mix');},[activeTrackId]);
  ...
  async function submitRevisions(){if(!revFiles.length||!project)return;setRevUploading(true);try{for(let i=0;i<revFiles.length;i++){const entry=revFiles[i];const tone=TONES[entry.tone];const ncId='rev-'+Date.now()+'-'+i;if(window.nc_startUpload)window.nc_startUpload(ncId,entry.name,project.id,project.title,entry.file.size);setRevStatus('Uploading '+(i+1)+'/'+revFiles.length+': '+entry.name);
      ...
      const gcsRes=await fetch('/api/gcs-upload',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fileName:entry.file.name,contentType:entry.file.type||'audio/wav',projectId:project.id})});
      const gcsData=await gcsRes.json();
      if(!gcsData.uploadUrl)throw new Error('No GCS upload URL');
      // Upload with XHR for progress tracking
      const publicUrl=await new Promise((resolve,reject)=>{
        const xhr=new XMLHttpRequest();
        xhr.open('PUT',gcsData.uploadUrl);
        xhr.setRequestHeader('Content-Type',entry.file.type||'audio/wav');
        xhr.upload.onprogress=e=>{if(e.lengthComputable&&window.nc_updateUpload)window.nc_updateUpload(ncId,e.loaded,e.total);};
        xhr.onload=()=>{if(window.nc_finishUpload)window.nc_finishUpload(ncId);resolve(gcsData.publicUrl||gcsData.uploadUrl.split('?')[0]);};
        xhr.onerror=()=>{if(window.nc_finishUpload)window.nc_finishUpload(ncId);reject(new Error('Upload failed'));};
        xhr.send(entry.file);
      });
      const peaks=entry.peaks?.length>0?entry.peaks:[];
      let trackId,revisionId;
      if(entry.matchedTrackId&&!entry.isNew){
        // Revision of existing track
        trackId=entry.matchedTrackId;
        const{data:existing}=await sb.from('revisions').select('version_number').eq('track_id',trackId).order('version_number',{ascending:false}).limit(1);
        const nextVer=(existing?.[0]?.version_number||1)+1;
        await sb.from('revisions').update({is_active:false}).eq('track_id',trackId);
        const{data:newRev}=await sb.from('revisions').insert({track_id:trackId,project_id:project.id,version_number:nextVer,label:'v'+nextVer,audio_url:publicUrl,mp3_url:publicUrl,tone_setting:entry.tone,tone_label:tone.short,is_active:true}).select('id').single();
        revisionId=newRev?.id;
        await sb.from('tracks').update({audio_url:publicUrl,peaks:peaks.length>0?peaks:undefined,tone_setting:entry.tone,tone_label:tone.short}).eq('id',trackId);
      }else{
        // New track — init-master will create the v1 revision
        const{data:newTrack}=await sb.from('tracks').insert({project_id:project.id,title:entry.name,audio_url:publicUrl,mp3_url:publicUrl,position:tracks.length+i,peaks,tone_setting:entry.tone,tone_label:tone.short}).select('id').single();
        trackId=newTrack?.id;
      }
      // Call init-master to kick off DawDreamer pipeline
      if(trackId){
        const initRes=await fetch('/api/init-master',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({trackId,projectId:project.id,...(revisionId?{revisionId,audioUrl:publicUrl,toneLabel:tone.short}:{})})});
        const initData=await initRes.json();
        if(initData.masterId&&window.nc_startMaster)window.nc_startMaster(initData.masterId,entry.name,project.id,entry.file.size);
      }
      if(entry.name.trim())setToneMemory(entry.name.trim(),entry.tone);
    }setShowRevModal(false);setRevFiles([]);setRevStatus('');await loadProject(project.id);}catch(e){setRevStatus('Error: '+e.message);}setRevUploading(false);}
