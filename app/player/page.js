...
/* Brighter color swatches â warmth (col) x loudness (row opacity) */
const TONE_BG=[
...
function Waveform({peaks, progress, notes, duration, onSeek}) {
  ...
  // Main draw effect â runs on mount and when peaks change
  useEffect(() => {
    ...
    function draw() {
      ...
      ctx.fillText('Processing audioâ¦ wonât be long', W / 2, H / 2);
      ctx.beginPath();
    ...
    // ResizeObserver â just let the draw loop handle resize naturally
...
/* ToneGrid â bright color swatches, no text in cells, X through used (color kept) */
function ToneGrid({value,usedTones=[],onChange,onSetAll,showSetAll}){
  ...
  return(<div className="tgm-wrap">
    <div className="tgm-axes"><span>â Warmer</span><span style={{margin:'0 auto',color:'var(--amber)',fontWeight:500,fontSize:10}}>TONE GRID</span><span>Brighter â</span></div>
    ...
    {usedTones.length>0&&<div style={{fontSize:10,color:'var(--t3)',marginTop:6}}>â Already mastered â crossed cells unavailable</div>}
    ...
    <div className={'td-modal'+(open?' td-open':'')}>
      <div className="td-header">
        ...
        <div className="td-title-block">
          ...
          {displayRev&&<div className="td-rev-row"><span className="td-rev-badge">{displayRev.label||'v1'}</span>{displayRev.tone_label&&<span className="td-tone-badge">{displayRev.tone_label}</span>}{revisions.length>1&&<button className="td-rev-switch" onClick={()=>setRevSwitcherOpen(v=>!v)}>{revisions.length} versions â¾</button>}</div>}
        </div>
      </div>
      ...
      {/* Master selector â shows preset pills for active revision */}
        ...
        <div className="td-master-pills">
          {[
            ...
            >
              ...
              {isPending&&<span className="td-master-pill-status">â³</span>}
              {isReady&&!isActive&&<span className="td-master-pill-status">â</span>}
              {isActive&&<span className="td-master-pill-status">â¶</span>}
            </button>);
          })}
        </div>
...
const [showInvite,setShowInvite]=useState(false);
  ...
  useEffect(()=>{
    ...
    const hlsUrl=activeSource==='master'&&activeMaster?.hls_url?activeMaster.hls_url:rev?.hls_url||track?.hls_url||null;
    ...
    el.pause();
    if(hlsUrl){
      // HLS path â instant start, FLAC quality
  ...
  // HLS polling â watches for hls_url to appear after encoding completes
  useEffect(()=>{
...
useEffect(()=>{setActiveSource('mix');},[activeTrackId]);
  ...
  return(<>
    ...
    /* TONE GRID â color swatches, X on used */
...
@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
    <div className="topbar"><div style={{display:'flex',alignItems:'center',gap:6,minWidth:0}}><a href="/" className="logo">maastr<em>.</em></a><span style={{color:'var(--border2)',fontSize:14,flexShrink:0}}>/</span><span className="breadcrumb">{project?.title||'â¦'}</span></div><div style={{display:'flex',alignItems:'center',gap:8}}><a href="/" className="back">â Dashboard</a>{user&&<NotificationCenter user={user}/>}<div style={{position:'relative'}}>
    ...
    <div className="ps-controls-bar">
      {activeMaster?.status==='ready'&&(<div className="ps-source-toggle"><button className={`ps-src-btn${activeSource==='mix'?' ps-src-active':''}`} onClick={()=>setActiveSource('mix')}>MIX</button><button className={`ps-src-btn${activeSource==='master'?' ps-src-active':''}`} onClick={()=>setActiveSource('master')}>MASTER</button></div>)}{activeMaster&&activeMaster.status!=='ready'&&(<span className="ps-mastering-lbl">Mastering…</span>)}<div className="ps-transport">
        ...
        <div style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',background:'var(--surf)',border:'1px solid var(--border2)',borderRadius:14,padding:28,width:360,maxWidth:'90vw',zIndex:201}}>
          ...
          {reportSent
            ? <div style={{textAlign:'center',padding:'20px 0'}}>
                ...
                <div style={{fontSize:13,color:'var(--amber)'}}>Thanks â we got it.</div>
