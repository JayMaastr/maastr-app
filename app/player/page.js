...
function TrackDetail({open,track,notes,currentTime,duration,progress,isPlaying,onTogglePlay,onSkip,onPrevTrack,onNextTrack,canPrev,canNext,onSeek,onClose,onPonRefresh,ost,onSeekToTime,activeMaster,user,...rest}){
  ...
  return(<>
    ...
    <div className={'td-modal'+(open?' td-open':'')}>
      ...
      <div className="td-comments-scroll">
        {notes.length>0?notes.map(n=>(<div key={n.id} data-comment-id={n.id} className="td-comment-card" onClick={()=>{setActiveNoteId(activeNoteId===n.id?null:n.id);n.timestamp_sec!=null&&onSeekToTime(n.timestamp_sec)}} style={activeNoteId===n.id?{background:'#1a1a1a',borderRadius:'8px',margin:'0 -4px',padding:'10px 8px'}:{}}>
          <div className="td-comment-meta"><div className="td-comment-avatar">{(n.author_name||'Y')[0].toUpperCase()}</div><span className="td-comment-name">{n.author_name||'You'}</span>{n.author_name===(user?.email?.split('@')[0]||'You')&&<span style={{color:'#fbbf24',fontSize:'9px',marginLeft:'4px'}}>(you)</span>}{n.timestamp_sec!=null&&<span className="td-comment-ts" onClick={() => { setActiveNoteId(activeNoteId===n.id?null:n.id); seekToTime(n.timestamp_sec); }} style={{cursor:'pointer'}}>{n.timestamp_label||fmt(n.timestamp_sec)}</span>}<span className="td-comment-date">{new Date(n.created_at).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</span></div>
          {editingNoteId===n.id?(<div style={{display:'flex',gap:'6px',alignItems:'flex-start'}}><textarea value={editText} onChange={e=>setEditText(e.target.value)} style={{flex:1,background:'#1a1a1a',border:'1px solid #333',borderRadius:'6px',color:'#ddd',padding:'6px 8px',fontSize:'12px',resize:'vertical',minHeight:'40px'}}/><div style={{display:'flex',flexDirection:'column',gap:'4px'}}><span onClick={async()=>{await supabase.from('notes').update({body:editText}).eq('id',n.id);setEditingNoteId(null);onPost?.();}} style={{fontSize:'10px',color:'#4ade80',cursor:'pointer',padding:'4px 8px',background:'rgba(74,222,128,0.12)',borderRadius:'4px'}}>Save</span><span onClick={()=>setEditingNoteId(null)} style={{fontSize:'10px',color:'#888',cursor:'pointer',padding:'4px 8px'}}>Cancel</span></div></div>):(<div className="td-comment-text">{editingNoteId===n.id?(<div style={{display:'flex',flexDirection:'column',gap:'6px'}}><textarea value={editText} onChange={e=>setEditText(e.target.value)} style={{background:'#1a1a1a',border:'1px solid #333',borderRadius:'6px',padding:'8px',color:'#ddd',fontSize:'12px',resize:'vertical',minHeight:'40px'}}/><div style={{display:'flex',gap:'6px'}}><span onClick={async(e)=>{e.stopPropagation();await supabase.from('notes').update({body:editText}).eq('id',n.id);setEditingNoteId(null);onRefresh();}} style={{fontSize:'10px',color:'#4ade80',cursor:'pointer'}}>Save</span><span onClick={(e)=>{e.stopPropagation();setEditingNoteId(null);}} style={{fontSize:'10px',color:'#888',cursor:'pointer'}}>Cancel</span></div>{replyingToId===n.id&&(<div onClick={(e)=>e.stopPropagation()} style={{display:'flex',gap:'6px',marginTop:'6px',alignItems:'center'}}><input value={replyText} onChange={e=>setReplyText(e.target.value)} placeholder="Write a reply..." style={{flex:1,background:'#1a1a1a',border:'1px solid #333',borderRadius:'16px',padding:'7px 12px',color:'#ddd',fontSize:'11px',outline:'none'}}/><span onClick={async()=>{if(!replyText.trim())return;await supabase.from('notes').insert({track_id:n.track_id,project_id:n.project_id,revision_id:n.revision_id,author_name:user?.email?.split('@')[0]||'You',timestamp_sec:n.timestamp_sec,timestamp_label:n.timestamp_label,body:replyText.trim(),resolved:false});setReplyText('');setReplyingToId(null);onRefresh();}} style={{color:'#fbbf24',fontWeight:'500',fontSize:'14px',cursor:'pointer',padding:'4px'}}>Send</span></div>)}</div>):n.body}</div>)}
          {activeNoteId===n.id&&(<div style={{display:'flex',gap:'12px',marginTop:'6px',flexWrap:'wrap'}}><span onClick={(e)=>{e.stopPropagation();setReplyingToId(replyingToId===n.id?null:n.id);setReplyText('');}} style={{fontSize:'10px',color:'#fbbf24',cursor:'pointer'}}>↩ Reply</span>{n.author_name===(user?.email?.split('@')[0]||'You')?(<><span onClick={(e)=>{e.stopPropagation();setEditingNoteId(n.id);setEditText(n.body);}} style={{fontSize:'10px',color:'#888',cursor:'pointer'}}>✎ Edit</span><span onClick={async(e)=>{e.stopPropagation();if(confirm('Delete this comment?')){await supabase.from('notes').delete().eq('id',n.id);onRefresh();}}} style={{fontSize:'10px',color:'#f87171',cursor:'pointer'}}>✕ Delete</span></>):(!n.resolved&&<span onClick={async(e)=>{e.stopPropagation();await supabase.from('notes').update({resolved:true}).eq('id',n.id);onRefresh();}} style={{fontSize:'10px',color:'#4ade80',cursor:'pointer'}}>✓ Mark complete</span>)}{n.resolved&&<span style={{fontSize:'9px',background:'rgba(74,222,128,0.12)',color:'#4ade80',padding:'1px 6px',borderRadius:'3px'}}>✓ Addressed</span>}</div>)}
